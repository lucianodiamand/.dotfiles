...
ssh $SSH_OPTS "${CI_USER}@${CP_IP}" \
  "APP_NS='${APP_NS}' OBS_NS='${OBS_NS}' bash -s" <<'REMOTE'
set -euo pipefail
export KUBECONFIG=/etc/rancher/k3s/k3s.yaml

: "${APP_NS:?missing APP_NS}"
: "${OBS_NS:?missing OBS_NS}"

kubectl create ns "${APP_NS}" >/dev/null 2>&1 || true

# (opcional) limpiar todo si querés ir a cero:
# kubectl -n "${APP_NS}" delete ingress/apps-ingress svc/svc-a svc/svc-b deploy/svc-a deploy/svc-b \
#   cm/demo-svc-a cm/demo-svc-b cm/demo-node-common --ignore-not-found=true
...
data:
  ...
  otel.js: |
    ...
    // En OTLP gRPC el exporter espera host:port (sin http://)
    const endpoint = process.env.OTEL_EXPORTER_OTLP_ENDPOINT || 'otel-collector.observability.svc.cluster.local:4317';
  ...
  template:
    ...
    spec:
      ...
      - name: src
        projected:
          sources:
          - configMap: { name: demo-node-common }
          - configMap: { name: demo-svc-b }
      - name: app
        emptyDir: {}
...
kubectl -n "${APP_NS}" rollout status deploy/svc-b --timeout=30m
kubectl -n "${APP_NS}" rollout status deploy/svc-a --timeout=30m
